...
/**
 * Retrieve a collection of tweets posted by everybody in reverse chronological
 * order (newest first). Both `from` and `to` are zero-based indexes, where 0 is
 * the first element, 1 is the next element and so on. They can also be negative
 * numbers indicating offsets from the end of the collection, with -1 being the
 * last element of the sorted set, -2 the penultimate element and so on.
 *
 * @param  {Number} from - Index from
 * @param  {Number} to - Index to
 * @param  {Function} fn - Callback gets two arguments `(err, tweets)` where
 * `tweets` is a list of tweets.
 */
Tweet.getGlobalTimeline = function(from, to, fn) {
  client.zrevrange('global_timeline', from, to, function(err, tweetIds) {
    if (err) {
      return fn(err);
    }
...
/**
 * Retrieve a collection of tweets posted by the user in reverse chronological
 * order (newest first). Both `from` and `to` are zero-based indexes, where 0 is
 * the first element, 1 is the next element and so on. They can also be negative
 * numbers indicating offsets from the end of the collection, with -1 being the
 * last element of the sorted set, -2 the penultimate element and so on.
 ...
 * `tweets` is a list of tweets.
 */
Tweet.getUserTimeline = function(userId, from, to, fn) {
  client.zrevrange('user:' + userId + ':user_timeline', from, to, function(err, tweetIds) {
    if (err) {
      return fn(err);
    }
    async.map(tweetIds, Tweet.get, fn);
...
/**
 * Retrieve a collection of tweets posted by the authenticating user and the
 * users they follow in reverse chronological order (newest first). Both `from`
 * and `to` are zero-based indexes, where 0 is the first element, 1 is the next
 * element and so on. They can also be negative numbers indicating offsets from
 * the end of the collection, with -1 being the last element of the sorted set,
 ...
 * `tweets` is a list of tweets.
 */
Tweet.getHomeTimeline = function(userId, from, to, fn) {
  client.zrevrange('user:' + userId + ':home_timeline', from, to, function(err, tweetIds) {
    if (err) {
      return fn(err);
    }
    async.map(tweetIds, Tweet.get, fn);
